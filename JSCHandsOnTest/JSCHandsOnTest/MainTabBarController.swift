...
class MainTabBarController: ESTabBarController, UITabBarControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        //let vc1 = TestViewController.loadFromNib()
        let nc1 = MainNavigationController(rootViewController: vc1)
        nc1.hidesBottomBarWhenPushed = true
        viewControllers = [nc1]
        
        shouldHijackHandler = {
            tabbarController, viewController, index in
            print(index)
            if (index == 0 || index == 1 || index == 2 || index == 3 || index == 4) && !UserDefaults.standard.bool(forKey: UD.isLoggedIn.key) {
                return true
            }
            return false
        }
        ...
        didHijackHandler = {
            ...
            if !UserDefaults.standard.bool(forKey: UD.isLoggedIn.key) {
                print(index, "did")
...
class MainIrregularityBasicContentView: ESTabBarItemContentView {
    
    override init(frame: CGRect) {
        super.init(frame: frame)
        
        textColor = UIColor.brownishGreyTwo
        highlightTextColor = UIColor.peacockBlue
        iconColor = UIColor.brownishGreyTwo
        highlightIconColor = UIColor.peacockBlue
    }
...
class MainIrregularityContentView: ESTabBarItemContentView {
    
    override init(frame: CGRect) {
        super.init(frame: frame)
        
        self.imageView.backgroundColor = UIColor.peacockBlue
    ...
    override func point(inside point: CGPoint, with event: UIEvent?) -> Bool {
        let p = CGPoint.init(
            x: point.x - imageView.frame.origin.x,
    ...
    private func playMaskAnimation(
        animateView view: UIView,
        target: UIView,
        completion: (() -> ())?) {
        
        view.center = CGPoint.init(
            x: target.frame.origin.x + target.frame.size.width / 2.0,
            y: target.frame.origin.y + target.frame.size.height / 2.0
        )
